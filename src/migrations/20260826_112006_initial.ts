...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  await db.run(sql`CREATE TABLE \`artiklid\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`pealkiri\` text NOT NULL,
  	\`kategooria\` text DEFAULT 'turuulevaated' NOT NULL,
  	\`kuupaev\` text NOT NULL,
  	\`kaanepilt_id\` integer,
  	\`tutvustus\` text NOT NULL,
  	\`sisu\` text,
  	\`legacy_html\` text,
  	\`slug\` text,
  	...
  	\`updated_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	\`created_at\` text DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')) NOT NULL,
  	FOREIGN KEY (\`kaanepilt_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE set null
  );
  `)
  await db.run(sql`CREATE INDEX \`artiklid_kaanepilt_idx\` ON \`artiklid\` (\`kaanepilt_id\`);`)
  await db.run(sql`CREATE INDEX \`artiklid_slug_idx\` ON \`artiklid\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`artiklid_updated_at_idx\` ON \`artiklid\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`artiklid_created_at_idx\` ON \`artiklid\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`objektid\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`pealkiri\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`objektid_thumbnail_idx\` ON \`objektid\` (\`thumbnail_id\`);`)
  await db.run(sql`CREATE INDEX \`objektid_slug_idx\` ON \`objektid\` (\`slug\`);`)
  await db.run(sql`CREATE INDEX \`objektid_updated_at_idx\` ON \`objektid\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`objektid_created_at_idx\` ON \`objektid\` (\`created_at\`);`)
  await db.run(sql`CREATE TABLE \`objektid_rels\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`order\` integer,
  	\`parent_id\` integer NOT NULL,
  	\`path\` text NOT NULL,
  	\`media_id\` integer,
  	FOREIGN KEY (\`parent_id\`) REFERENCES \`objektid\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`media_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE cascade
  );
  `)
  await db.run(sql`CREATE INDEX \`objektid_rels_order_idx\` ON \`objektid_rels\` (\`order\`);`)
  await db.run(sql`CREATE INDEX \`objektid_rels_parent_idx\` ON \`objektid_rels\` (\`parent_id\`);`)
  await db.run(sql`CREATE INDEX \`objektid_rels_path_idx\` ON \`objektid_rels\` (\`path\`);`)
  await db.run(sql`CREATE INDEX \`objektid_rels_media_id_idx\` ON \`objektid_rels\` (\`media_id\`);`)
  await db.run(sql`CREATE TABLE \`paringud\` (
  	\`id\` integer PRIMARY KEY NOT NULL,
  	\`nimi\` text NOT NULL,
  	\`email\` text NOT NULL,
  ...
  `)
  await db.run(sql`CREATE INDEX \`paringud_updated_at_idx\` ON \`paringud\` (\`updated_at\`);`)
  await db.run(sql`CREATE INDEX \`paringud_created_at_idx\` ON \`paringud\` (\`created_at\`);`)
  ...
  await db.run(sql`CREATE TABLE \`payload_locked_documents_rels\` (
  	...
  	FOREIGN KEY (\`artiklid_id\`) REFERENCES \`artiklid\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`objektid_id\`) REFERENCES \`objektid\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`paringud_id\`) REFERENCES \`paringud\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`pages_id\`) REFERENCES \`pages\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`posts_id\`) REFERENCES \`posts\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`media_id\`) REFERENCES \`media\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  	FOREIGN KEY (\`categories_id\`) REFERENCES \`categories\`(\`id\`) ON UPDATE no action ON DELETE cascade,
  ...
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_artiklid_id_idx\` ON \`payload_locked_documents_rels\` (\`artiklid_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_objektid_id_idx\` ON \`payload_locked_documents_rels\` (\`objektid_id\`);`)
  await db.run(sql`CREATE INDEX \`payload_locked_documents_rels_paringud_id_idx\` ON \`payload_locked_documents_rels\` (\`paringud_id\`);`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  await db.run(sql`DROP TABLE \`artiklid\`;`)
  await db.run(sql`DROP TABLE \`objektid\`;`)
  await db.run(sql`DROP TABLE \`objektid_rels\`;`)
  await db.run(sql`DROP TABLE \`paringud\`;`)
